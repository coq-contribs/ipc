...
From IPC Require Export Rules Weight.
...
elimtype False.
apply (Nat.nlt_0_r (weight_gamma (vlist2hlist gamma))); assumption.
...
simpl in |- *; apply Nat.succ_lt_mono; assumption.
(* b=(OrF b0 b1) *)
intros b0 b1 lt_weight.
elim (int_succ j).
intros j1 less1.
...
simpl in |- *; apply Nat.lt_succ_diag_r.
(* a=(OrF a0 a1) *)
intros a0 a1 lt_weight.
elim (int_succ j).
intros j1 less1.
...
apply
 Nat.le_lt_trans with (weight (Imp Falsum c) + weight_gamma (vlist2hlist gamma));
 try assumption.
simpl in |- *.
apply (Nat.add_le_mono_r 1 (weight c) (weight_gamma (vlist2hlist gamma))).
apply weight_ge_1.
...
apply my_lt_weak.
apply -> Nat.succ_lt_mono.
 rewrite (Nat.add_succ_r (weight_neg a) 0).
simpl in |- *.
apply -> Nat.succ_lt_mono.
...
apply Nat.succ_lt_mono.
 rewrite (plus_O (weight_neg a)).
 rewrite (Nat.add_succ_r (weight_neg b) 0).
simpl in |- *.
 rewrite (plus_O (weight_neg b)).
 rewrite (Nat.add_succ_r (weight_neg a) (weight_neg b)).
simpl in |- *.
apply Nat.succ_lt_mono.
apply Nat.lt_succ_diag_r.
(* c=(OrF c0 c1) *)
...
simpl in |- *; fold weight_neg in |- *.
apply Nat.succ_lt_mono.
 rewrite (Nat.add_comm (weight c) (S (weight_neg b0 + weight_neg b1 + 1))).
simpl in |- *.
apply Nat.succ_lt_mono.
 rewrite (Nat.add_succ_r (weight_neg b0 + weight_neg b1) 0).
simpl in |- *.
apply Nat.succ_lt_mono.
...
simpl in |- *; fold weight_neg in |- *.
 rewrite (plus_O (weight_neg b0 + weight_neg b1)).
 rewrite
 (Nat.add_comm (weight c) (S (S (S (S (weight_neg b0 + weight_neg b1))))))
 .
simpl in |- *.
apply Nat.succ_lt_mono.
apply Nat.succ_lt_mono.
...
generalize (weight_neg (Imp b0 b1)); intros m.
 rewrite (Nat.add_succ_r m 0).
simpl in |- *.
 rewrite (plus_O m).
 rewrite (Nat.add_comm (weight c) (S m)).
simpl in |- *.
apply Nat.succ_lt_mono.
apply Nat.lt_succ_diag_r.
...
generalize (weight c); intro cn.
 rewrite <- (Nat.add_assoc (weight_neg (Imp (Atom j) b)) cn a).
 rewrite (Nat.add_comm cn a).
 rewrite (Nat.add_assoc (weight_neg (Imp (Atom j) b)) a cn).
apply Nat.le_lt_trans with (S (S (S (weight_neg b))) + a + cn).
...
intros goal gamma work context j lt_weight.
elimtype False.
apply (Nat.nlt_0_r (weight_goal goal)); assumption.
