...
From Coq Require Export Lt Le.
From IPC Require Export Regular_Avl Le_Ks.
...
generalize (nweight_dni dni); clear dni; intros dni.
 rewrite (Nat.add_assoc ni n dni).
 rewrite (Nat.add_comm ni n).
trivial.
Qed.
...
generalize (nweight (nimp2form x)); clear x; intro x.
 rewrite (Nat.add_comm x W).
 rewrite <- (Nat.add_assoc W x (ds + (ni + ai))).
apply plus_reg.
 rewrite (Nat.add_assoc x ds (ni + ai)).
 rewrite (Nat.add_comm x ds).
 rewrite <- (Nat.add_assoc ds x (ni + ai)).
apply plus_reg.
apply Nat.add_assoc.
Qed.
...
fold nweight_work in |- *.
fold nweight_ds in |- *.
 rewrite (Nat.add_succ_r (nweight_work work) (nweight_ds ds + n)).
trivial.
Qed.
...
generalize (nweight (nf2form (AImp i b))); clear i b; intro a.
 rewrite (Nat.add_comm a work).
 rewrite <- (Nat.add_assoc work a (ds + (ni + ai))).
apply plus_reg.
 rewrite (Nat.add_shuffle3 a ds (ni + ai)).
apply plus_reg.
 rewrite (Nat.add_shuffle3 a ni ai).
apply plus_reg.
trivial.
assumption.
Qed.
...
generalize (nweight_work work); clear work; intro W.
 rewrite (Nat.add_comm (nweight_work bs) W).
 rewrite <-
 (Nat.add_assoc W (nweight_work bs) (ds + (ni + nweight_ai ai'))).
apply Nat.add_lt_mono_l.
 rewrite (Nat.add_shuffle3 (nweight_work bs) ds (ni + nweight_ai ai')).
apply Nat.add_lt_mono_l.
 rewrite (Nat.add_shuffle3 (nweight_work bs) ni (nweight_ai ai')).
apply Nat.add_lt_mono_l.
...
generalize (nweight (nf2form b)); clear b; intros b.
 rewrite <- (Nat.add_assoc b (nweight_ni ni1) (nweight_dni dni1)).
...
generalize (nweight_work work); clear work; intros W.
 rewrite <- (Nat.add_assoc b n ai).
generalize (n + ai); clear n ai; intros n.
 rewrite (Nat.add_succ_r ds (S (b + n))).
simpl in |- *.
 rewrite (Nat.add_succ_r ds (b + n)).
simpl in |- *.
 rewrite (Nat.add_succ_r W (S (ds + (b + n)))).
simpl in |- *.
 rewrite (Nat.add_succ_r W (ds + (b + n))).
simpl in |- *.
apply S_reg.
apply S_reg.
 rewrite (Nat.add_comm b W).
 rewrite <- (Nat.add_assoc W b (ds + n)).
apply plus_reg.
 rewrite (Nat.add_assoc b ds n).
 rewrite (Nat.add_comm b ds).
...
generalize (nweight_ni ni2); clear eqv12 ni2; intros ni2.
 rewrite <- (Nat.add_assoc b ni2 ai).
generalize (ni2 + ai); clear ni1 dni1 ni2 ai n; intros n.
 rewrite (Nat.add_comm b W).
 rewrite <- (Nat.add_assoc W b (ds + n)).
 rewrite (Nat.add_assoc b ds n).
 rewrite (Nat.add_comm b ds).
 rewrite <- (Nat.add_assoc ds b n).
 rewrite (Nat.add_succ_r ds (S (b + n))).
simpl in |- *.
 rewrite (Nat.add_succ_r ds (b + n)).
simpl in |- *.
...
apply Nat.succ_lt_mono.
apply Nat.le_lt_trans with (nweight_Sequent work ((i, j) :: ds) ni1 ai);
 try assumption; clear lt1.
unfold nweight_Sequent in |- *.
generalize (nweight_ai ai); clear ai; intros ai.
