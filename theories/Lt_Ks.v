...
From Coq Require Export Le Lt.
From IPC Require Export Le_Ks.
...
apply Nat.le_trans with (count_undecs ni2).
assumption.
...
apply Nat.le_lt_trans with (count_undecs ni); assumption.
apply Nat.lt_succ_diag_r.
apply sih.
apply Nat.le_lt_trans with (count_undecs ni).
apply le_ks_le_count_undecs with dni1 dni; assumption.
assumption.
apply Nat.lt_le_trans with (length ni).
assumption.
