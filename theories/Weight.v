...
clear ih_a ih_b.
case a; clear a; intros; simpl in |- *; apply le_n_S; apply Nat.le_0_l.
intros a ih_a b ih_b.
simpl in |- *.
fold weight_neg in |- *.
apply Nat.le_trans with (weight b); try assumption.
apply (Nat.add_le_mono_r 0 (weight_neg a) (weight b)).
apply Nat.le_0_l.
Qed.
...
intros; apply Nat.le_trans with 2; apply Nat.le_succ_diag_r.
intros; apply le_n.
intros; apply le_n.
intros; apply le_n.
Qed.
...
apply Nat.le_lt_trans with (weight_gamma (b :: gamma)); try assumption.
simpl in |- *.
apply Nat.le_succ_l.
...
apply Nat.succ_lt_mono.
apply Nat.le_lt_trans with (weight_gamma (c :: gamma)); try assumption.
simpl in |- *.
apply Nat.le_succ_l.
apply Nat.add_lt_mono_r.
assumption.
...
apply lt_plus_assoc_l.
apply Nat.succ_lt_mono.
apply Nat.le_lt_trans with (weight_gamma (d :: gamma)); try assumption.
simpl in |- *.
apply Nat.le_succ_l.
apply Nat.add_lt_mono_r.
assumption.
